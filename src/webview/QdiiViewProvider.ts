...
const FUND_FULL_POSITION_PARAMS: Record<string, { year: string; month: string }> = {
...
export class QdiiViewProvider implements vscode.WebviewViewProvider {
  ...
  public async refresh(): Promise<void> {
    ...
    try {
      ...
      this.output.appendLine('posting data: algorithm1=' + payload.modes.algorithm1.length + ', algorithm3=' + payload.modes.algorithm3.length);
      await this.view.webview.postMessage({ type: 'data', payload });
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      this.output.appendLine('refresh error: ' + message);
      await this.view.webview.postMessage({ type: 'error', payload: { message } });
    }
  }
...
function runLiveDataScript(extensionPath: string): Promise<AnyFund> {
  return new Promise((resolve, reject) => {
    ...
    childProcess.execFile('node', [scriptPath, extensionPath], {
      cwd: extensionPath,
      timeout: 45000,
      ...
      try {
        ...
        reject(new Error('live-data parse failed: ' + String(parseError) + '\n' + stdout.slice(0, 500)));
      }
    });
  });
}
function buildEstimatedFund(source: AnyFund, holdings: Holding[], mode: 'algorithm1' | 'algorithm3', sourceName: string): AnyFund {
  const available = holdings.filter((holding) => typeof holding.returnPct === 'number' && Number.isFinite(holding.returnPct));
  const knownWeightPct = sum(available.map((holding) => holding.weightPct));
  const knownContributionPct = sum(available.map((holding) => holding.contributionPct ?? 0));
  const stockStyleReturnPct = knownWeightPct > 0 ? (knownContributionPct / knownWeightPct) * 100 : 0;
  const equityLikePct = Number(source.assetAllocation?.equityLikePct ?? source.assetAllocation?.stockPct ?? 0);
  ...
  return normalizeFund({
    ...
    latestEstimate: {
      ...
      rawEstimatePct,
      tunedEstimatePct,
      missing: holdings.filter((holding) => typeof holding.returnPct !== 'number').map((holding) => holding.secid),
    },
    calculatedMetrics: {
...
function buildFallbackHoldings(fund: AnyFund): Holding[] {
  ...
  const stockStyleReturnPct = Number(fund.latestEstimate?.stockStyleReturnPct ?? 0);
  if (!topWeightPct) {
    return [];
  }
  return [{
...
function buildHybridHoldings(fullHoldings: Holding[], latestTop10: Holding[]): Holding[] {
...
function applyQuotes(holdings: Holding[], quotes: Map<string, number>): Holding[] {
  return holdings.map((holding) => {
    const returnPct = holding.returnPct ?? quotes.get(holding.secid);
    return {
      ...holding,
      returnPct: typeof returnPct === 'number' ? returnPct : null,
      contributionPct: typeof returnPct === 'number' ? (holding.weightPct * returnPct) / 100 : null,
    };
  });
}

async function fetchFundHoldings(code: string, params: { topline: number; year: string; month: string }): Promise<Holding[]> {
...
async function fetchQuoteReturns(secids: string[]): Promise<Map<string, number>> {
  ...
  for (const group of chunk(secids, 80)) {
    ...
    const json = JSON.parse(await httpsGetText(url, 'https://quote.eastmoney.com/')) as AnyFund;
...
function parseHoldingRows(html: string): Holding[] {
  ...
  return rows.map((row) => {
    ...
    const cells = Array.from(row.matchAll(/<td class='toc'[^>]*>([\s\S]*?)<\/td>/g))
  ...
  }).filter((holding) => holding.rank > 0 && holding.secid && holding.weightPct > 0);
}
...
function httpsGetText(url: string, referer: string): Promise<string> {
  return new Promise((resolve, reject) => {
